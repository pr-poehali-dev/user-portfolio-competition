...
const ContestsList = () => {
  ...
  // Все конкурсы с информацией о назначенном жюри
  const allContests = [
    {
      id: 1,
      ...
      assignedJury: ["jury-1", "jury-2", "jury-3"],
    },
    {
      id: 2,
    ...
    },
    {
      ...
      assignedJury: ["jury-2", "jury-3"],
    },
  ];

  // Фильтруем конкурсы только для текущего пользователя жюри
  const contests = allContests.filter(
    (contest) => user && contest.assignedJury.includes(user.id),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <h3 className="text-lg font-medium mb-2">Назначенные конкурсы</h3>
        <p className="text-sm text-gray-600">
          Вы назначены жюри в {contests.length} конкурсах
        </p>
      </div>
        ...
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-gray-500">
              Вы пока не назначены жюри ни в одном конкурсе
            </p>
          </CardContent>
        </Card>
      ) : (
